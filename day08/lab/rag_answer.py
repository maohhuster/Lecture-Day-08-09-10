...
def rerank(
    ...
    # Build chunk list cho LLM chấm điểm relevance
    chunk_descriptions = []
    ...
    rerank_prompt = f"""You are a relevance judge. Given a question and a list of text chunks, score each chunk's relevance to the question from 0 to 10.
...
Return ONLY a JSON array of objects with "index" (int) and "score" (int 0-10), sorted by score descending.
Example: [{{"index": 2, "score": 9}}, {{"index": 0, "score": 5}}]
"""

    raw = call_llm(rerank_prompt)
    ...
    try:
        ...
        rankings = _json.loads(raw[start:end])
        ...
        print(f"[rerank] LLM parse error, fallback to top_k: {e}")
...
def compare_retrieval_strategies(query: str) -> None:
    ...
    # Hybrid + Rerank variant
    print(f"\n--- Strategy: hybrid + rerank ---")
    try:
        result = rag_answer(query, retrieval_mode="hybrid", use_rerank=True, verbose=False)
        print(f"Answer: {result['answer']}")
        print(f"Sources: {result['sources']}")
    except Exception as e:
        print(f"Lỗi: {e}")
